...
def noniid(dataset, dataset_name, num_users, badclient_prop, num_cat):
    ...
    for client_id in iid_clients:
        for cls in classes:
            ...
            indices_to_remove = np.random.choice(cls_indices, size=num_to_remove_per_class, replace=False).tolist()
            dict_users[client_id] = [idx for idx in dict_users[client_id] if idx not in indices_to_remove]
