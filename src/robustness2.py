...
def train_subset(j, args, train_dataset, valid_dataset, test_dataset, user_groups):
    ...
    abv_simple, abv_hessian, shapley_values, influence_values, runtimes = train_global_model(args, global_model, train_dataset, valid_dataset, test_dataset, user_groups, device, j)
    return (j, abv_simple, abv_hessian, shapley_values, influence_values, runtimes)
...
if __name__ == '__main__':
    ...
    for j, abv_simple, abv_hessian, shapley_values, influence_values, runtimes in results_list:
        logger.info(f'ABV Simple {j}: {abv_simple}')
        logger.info(f'ABV Hessian {j}: {abv_hessian}')
        logger.info(f'Shapley Values {j}: {shapley_values}')
        logger.info(f'Influence Values {j}: {influence_values}')
        logger.info(f'Runtimes {j}: {runtimes}')
